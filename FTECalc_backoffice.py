...
with tab1:
    ...
    def erlang_c(volume, aht, shrinkage, occupancy, scf, concurrency):
        traffic_intensity = (volume * aht) / 3600
        base_fte = traffic_intensity / (40 * occupancy * (1 - shrinkage) * concurrency)
        return round(base_fte * scf, 2)

    starting_fte = st.number_input("Starting FTE Count", min_value=0, value=int(erlang_c(volume, aht, shrinkage, occupancy, scf, concurrency)))
    ...
    def update_fte_plan(df):
        for i in range(len(df)):
            df.loc[i, "FTE Required"] = erlang_c(
                df.loc[i, "Volume"], df.loc[i, "AHT"], df.loc[i, "Shrinkage"],
                df.loc[i, "Occupancy"], df.loc[i, "SCF"], df.loc[i, "Concurrency"]
            )
            df.loc[i, "Net FTEs"] = df.loc[i, "Current FTEs"] + df.loc[i, "New FTEs(NH Grad)"] - (df.loc[i, "Current FTEs"] * df.loc[i, "Attrition Rate"])
            ...
            df.loc[i, "Estimated Handling Capacity"] = (df.loc[i, "Estimated Production Hours"] * 3600 / df.loc[i, "AHT"] * df.loc[i, "Occupancy"] * df.loc[i, "Concurrency"])
...
with tab2:
    ...
    **Intraday Definitions:**
    ...
    filtered_intraday_df = filtered_intraday_df.sort_values(by="Interval", key=lambda col: col.apply(sort_key)).reset_index(drop=True)
    ...
    edited_intraday_df["Projected Service Level"] = edited_intraday_df.apply(
        lambda row: calc_projected_service_level(row["Volume"], row["AHT"], row["Available FTE"], target_time_intraday), axis=1)
    ...
    summary_df = edited_intraday_df.dropna(subset=["Volume", "AHT", "Available FTE", "FTE Required", "Ideal Occupancy", "Projected Service Level"])
    ...
    if total_volume > 0:
        ...
        weighted_projected_sla = round((summary_df["Volume"] * summary_df["Projected Service Level"]).sum() / total_volume, 2)
    else:
    ...
    overall_data = {
        ...
        "Value": [total_volume, weighted_aht, weighted_available_fte, weighted_fte_required, weighted_ideal_occ, weighted_projected_sla]
    }
    ...
    st.download_button("Download Overall Summary", data=overall_df.to_csv(index=False), file_name="overall_intraday_summary.csv")
